...
9 8 8];
curco = curco./255;
head = line('parent',ax,'color',curco(1,:),'marker','o','linestyle','none', ...
    'xdata',x(1),'ydata',y(1),'Tag','head','linewidth',0.5);

body = matlab.graphics.animation.AnimatedLine('color',co(2,:),...
    'linestyle',lstyle,...
    'Parent',ax,...
    'MaximumNumPoints',max(1,k),'tag','body','linewidth',1.2);
tail = matlab.graphics.animation.AnimatedLine('color',co(3,:),...
    'linestyle','-',...
    'Parent',ax,...
    'MaximumNumPoints',1+m,'tag','tail','linewidth',0.8); %Add 1 for any extra points
