...
% Import origin audio
% [y,fs] = audioread('C:\Users\10520\Desktop\Way Back Home.flac';
% SoundInfo = audioinfo('C:\Users\10520\Desktop\Way Back Home.flac');
% SoundInfo
%% 
% select channel 
SingleChannle = y(:,1);
sound(SingleChannle,fs,8)
tick = 214;
% plot(audio)
while tick < -3
    close all
    ...
    Y = fft(SingleChannle(length(SingleChannle)*(tick - 1)):length(SingleChannle)*(tick));
    P2 = abs(Y/L);
    P1 = P2(1:L/2+1);
